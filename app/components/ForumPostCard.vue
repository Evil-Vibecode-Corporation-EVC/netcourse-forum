<template>
  ...
  >
    ...
    <div class="flex flex-col sm:flex-row sm:items-start gap-4">
      ...
      <div v-if="imageAttachments.length" class="hidden sm:block w-56 lg:w-64 shrink-0" @click.stop>
        <div class="relative overflow-hidden rounded-xl bg-slate-800" :class="imageAttachments.length === 1 ? 'aspect-[4/3]' : 'aspect-square'">
          <div v-if="imageAttachments.length === 1" class="w-full h-full">
            <img :src="imageAttachments[0].r2Key" :alt="imageAttachments[0].fileName" class="w-full h-full object-cover transition-transform duration-300 hover:scale-105 cursor-pointer" loading="lazy" />
          </div>
          <div v-else-if="imageAttachments.length === 2" class="flex flex-col w-full h-full gap-0.5">
            <div v-for="att in imageAttachments.slice(0, 2)" :key="att.id" class="flex-1 relative overflow-hidden">
              <img :src="att.r2Key" :alt="att.fileName" class="w-full h-full object-cover transition-transform duration-300 hover:scale-105 cursor-pointer" loading="lazy" />
            </div>
          </div>
          <div v-else-if="imageAttachments.length === 3" class="flex flex-col w-full h-full gap-0.5">
            <div class="flex-1 relative overflow-hidden">
              <img :src="imageAttachments[0].r2Key" :alt="imageAttachments[0].fileName" class="w-full h-full object-cover transition-transform duration-300 hover:scale-105 cursor-pointer" loading="lazy" />
            </div>
            <div class="h-1/3 flex gap-0.5">
              <div v-for="att in imageAttachments.slice(1, 3)" :key="att.id" class="flex-1 relative overflow-hidden">
                <img :src="att.r2Key" :alt="att.fileName" class="w-full h-full object-cover transition-transform duration-300 hover:scale-105 cursor-pointer" loading="lazy" />
              </div>
            </div>
          </div>
          <div v-else class="grid grid-cols-2 w-full h-full gap-0.5">
            <div v-for="(att, idx) in imageAttachments.slice(0, 4)" :key="att.id" class="relative overflow-hidden">
              <img :src="att.r2Key" :alt="att.fileName" class="w-full h-full object-cover transition-transform duration-300 hover:scale-105 cursor-pointer" loading="lazy" />
              <div v-if="idx === 3 && imageAttachments.length > 4" class="absolute inset-0 bg-slate-900/70 flex items-center justify-center">
      ...
      <div v-if="imageAttachments.length" class="sm:hidden -mx-5 overflow-hidden" @click.stop>
        <div class="flex gap-0.5">
          <div
            v-for="att in imageAttachments.slice(0, 3)"
            :key="att.id"
            class="flex-1 h-32 relative overflow-hidden"
          >
            <img :src="att.r2Key" :alt="att.fileName" class="w-full h-full object-cover cursor-pointer" loading="lazy" />
          </div>
          <div
            v-if="imageAttachments.length > 3"
            class="flex-1 h-32 bg-slate-800 flex items-center justify-center"
          >
            <span class="text-slate-400 font-mono text-sm">+{{ imageAttachments.length - 3 }}</span>
          </div>
        </div>
      </div>
    </div>
