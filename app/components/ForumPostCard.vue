<template>
  ...
  >
    ...
    <div class="flex items-start gap-3 mb-3">
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2 flex-wrap">
          <span class="text-emerald-400 font-mono text-sm font-semibold">
          ...
          <span
            v-if="isCoursePost"
            class="ml-2 sm:ml-3 px-1.5 sm:px-2.5 py-0.5 bg-emerald-500/10 border border-emerald-400 rounded-full text-emerald-300 font-mono text-[10px] sm:text-xs font-bold leading-tight whitespace-nowrap"
          >
    ...
    <h3 class="text-white font-semibold text-base mb-2 transition-colors leading-snug line-clamp-2" :class="isCoursePost ? 'group-hover:text-emerald-300' : 'group-hover:text-emerald-400'">
      {{ post.title }}
    </h3>
    ...
    <div v-if="imageAttachments.length" class="mb-3 -mx-5 sm:mx-0" @click.stop>
      <div class="relative overflow-hidden sm:rounded-xl bg-slate-800/60 max-h-[320px]">
        <div v-if="imageAttachments.length === 1" class="w-full aspect-video">
          <img :src="imageAttachments[0].r2Key" :alt="imageAttachments[0].fileName"
            class="w-full h-full object-cover transition-transform duration-300 hover:scale-[1.02] cursor-pointer" loading="lazy" />
        </div>
        <div v-else-if="imageAttachments.length === 2" class="grid grid-cols-2 w-full h-44 sm:h-52 gap-px">
          <div v-for="att in imageAttachments.slice(0, 2)" :key="att.id" class="relative overflow-hidden">
            <img :src="att.r2Key" :alt="att.fileName" class="w-full h-full object-cover transition-transform duration-300 hover:scale-[1.02] cursor-pointer" loading="lazy" />
          </div>
        </div>
        <div v-else-if="imageAttachments.length === 3" class="grid grid-cols-2 w-full h-48 sm:h-56 gap-px">
          <div class="row-span-2 relative overflow-hidden">
            <img :src="imageAttachments[0].r2Key" :alt="imageAttachments[0].fileName" class="w-full h-full object-cover transition-transform duration-300 hover:scale-[1.02] cursor-pointer" loading="lazy" />
          </div>
          <div v-for="att in imageAttachments.slice(1, 3)" :key="att.id" class="relative overflow-hidden">
            <img :src="att.r2Key" :alt="att.fileName" class="w-full h-full object-cover transition-transform duration-300 hover:scale-[1.02] cursor-pointer" loading="lazy" />
          </div>
        </div>
        <div v-else class="grid grid-cols-2 w-full h-48 sm:h-56 gap-px">
          <div v-for="(att, idx) in imageAttachments.slice(0, 4)" :key="att.id" class="relative overflow-hidden">
            <img :src="att.r2Key" :alt="att.fileName" class="w-full h-full object-cover transition-transform duration-300 hover:scale-[1.02] cursor-pointer" loading="lazy" />
            <div v-if="idx === 3 && imageAttachments.length > 4" class="absolute inset-0 bg-slate-900/80 flex items-center justify-center backdrop-blur-sm">
              <span class="text-white font-mono text-lg font-bold">+{{ imageAttachments.length - 4 }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="flex items-center gap-4 text-xs font-mono text-slate-500 pt-3 border-t border-slate-800/80">
